...
export function getGradeLetter(marks, total) {
  if (!total || total <= 0) return "F";
  return getGradeFromPercentage((marks / total) * 100).grade;
}

export function getGradeColor(marks, total) {
  if (!total || total <= 0) return "#ef4444";
  const percentage = (marks / total) * 100;
  if (percentage >= 86) return "#22c55e"; // A / A+
  if (percentage >= 74) return "#3b82f6"; // B range
  if (percentage >= 62) return "#8b5cf6"; // C range
  if (percentage >= 50) return "#f59e0b"; // D range
  return "#ef4444"; // F
}
