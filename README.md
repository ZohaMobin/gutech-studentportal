# Gutech LMS — Student Portal

React web app (Create React App) for **students**.
Students: marks, attendance, class timetable and transcript.
...
- **This portal (https://gutech-studentportal-ten.vercel.app)** is hosted on **Vercel**, under the account **zohamobin@gmail.com**.
...
- The `.env` file committed in this repo contains only a `localhost` placeholder and is **not** what
  the live site uses.
- If the browser shows a **CORS error** on login, this portal's address (`https://gutech-studentportal-ten.vercel.app`, exactly, with
...
- There is **no student sign-up**. Accounts are created by an administrator through the student
  import in the admin portal. Students log in with their roll number or email.
- Grades shown here come from the backend; do not add grade calculations to the browser.
